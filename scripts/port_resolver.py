...
class PortResolver:
    ...
    def resolve(self, raw: str) -> Tuple[str, str, str, str]:
        ...
        if re.match(r"^[A-Z0-9]{4,5}$", code_clean):
            ...
            # 可能是合法代码但字典里没有 — 这里宽松返回 (forward-compat),
            # 调用方若需严格命中 (如 export_cw._prefilter_rows 沃行闸门),
            # 应自行合并 by_code ∪ alias_index.values() 二次校验, 不要仅用 by_code
            # (alias-only 真实港口如 IDJKT 雅加达会被误杀).
            return (code_clean, "high", "ports.json", original)
