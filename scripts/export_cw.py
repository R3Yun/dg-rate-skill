...
# v3.10.6.2 (P0 沃行对齐): 预过滤失败异常 — main() 应捕获并拒绝写 .xls, 不 fallback legacy.
class _PrefilterError(ValueError):
    """沃行模板导出预过滤失败 (POL/POD JSON 匹配失败, 整批拒导)."""
    pass
...
META_ROW_5 = ["税率Cost(VAT)", "0%", "", "A列为空，其他列有数据将被当作注释，扫描价格时直接跳过", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""]
META_ROW_6 = ["税率Sell(VAT)", "0%", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""]
...
def _fmt_date(v) -> str:
    ...
    v3.10.6.2 D96-fix (2026-09-19): ISO 8601 的 T 分隔与空格分隔两种写法都取日期部分
    (外部输入可能是 `2026-09-20T00:00:00.000+08:00` / `2026-09-20 00:00:00` 或紧凑
    `20260920T00:00:00.000+08:00`)。无法归一为 YYYY-MM-DD 的输入原样返回。
    """
    if not v: return ""
    s = str(v).strip()
    if "T" in s or " " in s:
        s = s.replace(" ", "T").split("T")[0]
    # 已是 YYYY-MM-DD
...
def build_row(entry: NormalizedRateEntry, extra_dg_list: List[Dict[str, Any]] = None) -> Dict[str, str]:
    ...
    return {
        ...
        # v3.10.6.2: P/C 空时默认填 "Both" (沃行模板约定); Booking Agent 同上用 _get_booking_agent.
        "P/C": entry.pc or "Both",
        # v3.10.6.2: Carrier 走 _get_carrier 解析为 CargoWare 代码; 未匹配返回空 (由 prefilter 剔除).
        "Carrier": _get_carrier(entry),
        ...
        "Share": "",
        "Trend": "",
        "Sale State": "",  # D96-fix (2026-09-19): 按老板要求留空（不映射飞书状态字段）
...
def _get_carrier(entry) -> str:
    """v3.10.6.2: 沃行模板 Carrier 列 = CargoWare 代码 (镜像 _get_booking_agent).

    复用 booking_agent_master._code_* 反向索引 (code → [name list]):
      ① entry.carrier 直接查 _code_* 索引:
         - 已是 CargoWare 代码 (key in _code_cns) → 直接返回
         - 是名称/简称/英文名 (value in _code_cns/_code_shorts/_code_ens) → 返回对应 code
      ② 失败 fallback: master.resolve(carrier) → 中文名 → resolve_code(name)

    多客商撞码 (HBHY/SITC/AMHKGS) → 记入待确认清单, 返回空.
    """
    carrier = (getattr(entry, "carrier", "") or "").strip()
    if not carrier:
        return ""
    ...
    if nu in master._code_cns:
        matched_code = nu
    else:
        for code, names in master._code_cns.items():
            if carrier in names:
                matched_code = code
                break
        if matched_code is None:
            for code, names in master._code_shorts.items():
                if any(n.upper() == nu for n in names):
                    matched_code = code
                    break
        if matched_code is None:
            for code, names in master._code_ens.items():
                if any(n.upper() == nu for n in names):
                    matched_code = code
                    break

    if matched_code:
        if matched_code in master.ambiguous_codes:
            _record_ba_confirm(carrier, "", "Carrier 多客商共用代码, 需业务确认")
            return ""
        return matched_code

    name, st = master.resolve(carrier)
    if st == "ok" and name:
        code, code_st = master.resolve_code(name)
        if code_st == "ok":
            return code
        if code_st == "ambiguous":
            _record_ba_confirm(carrier, name, "Carrier 多客商共用代码, 需业务确认")
            return ""
    elif st == "ambiguous":
        _record_ba_confirm(carrier, "", "Carrier 多客商共用代码, 需业务确认")
        return ""

    _record_ba_confirm(carrier, "", "Carrier 未匹配到 CargoWare 代码")
    return ""


def _prefilter_rows(rows: List[Dict[str, str]],
                     entries: Optional[List["NormalizedRateEntry"]] = None) -> List[Dict[str, str]]:
    """v3.10.6.2: 沃行模板导出预过滤 (P0 沃行对齐).

    严格区分两套规则:
      ① POL/POD 任一未在 port_resolver (含子串模糊) 命中 → raise _PrefilterError, 整批拒导
      ② Carrier 未匹配 (row["Carrier"] 为空) → 剔除该条, stdout 日志
      ③ Booking Agent 未匹配 (row["Booking Agent"] 为空) → 剔除该条, stdout 日志

    Args:
      rows: List[Dict] — build_row 输出 (必须有 POL/POD/Carrier/Booking Agent 列)
      entries: 平行 entries 数组 (可选, 用于 skip 日志记录原始 carrier/ba)
               与 rows 一一对应; 缺省时日志只用 row 解析后的值 (空字符串)

    Returns:
      过滤后的 rows (仅剔除②③; ①抛异常不返回).
    """
    ...
    for i, row in enumerate(rows):
        ...
        if not pol or not pod:
            raise _PrefilterError(
                f"POL/POD JSON 匹配失败, 整批拒导: row {i+1} POL={pol!r} POD={pod!r}"
            )
        pol_code, pol_conf, _, _ = resolver.resolve(pol)
        pod_code, pod_conf, _, _ = resolver.resolve(pod)
        # v3.10.6.2 patch: port_resolver 内部两套字典 (by_code 899 ∪ alias 82 = 913)
        # 存在 alias-only 真实港口 (如 IDJKT 雅加达). prefilter 作为最后一道闸门,
        # 必须用并集当系统"已知港口全集", 既不误杀 alias-only 真实港口,
        # 也拒绝 ZZZZZ 这类幽灵码. 文案保持原文以让既有 4 条断言继续通过.
        known_codes = set(resolver.by_code) | set(resolver.alias_index.values())
        if pol_code not in known_codes or pod_code not in known_codes:
            raise _PrefilterError(
                f"POL/POD JSON 匹配失败, 整批拒导: row {i+1} POL={pol!r}({pol_conf}) "
                f"POD={pod!r}({pod_conf})"
            )

    kept: List[Dict[str, str]] = []
    for i, row in enumerate(rows):
        carrier_code = (row.get("Carrier") or "").strip()
        ba_code = (row.get("Booking Agent") or "").strip()
        e = entries[i] if entries is not None and i < len(entries) else None
        orig_carrier = (getattr(e, "carrier", "") or "") if e else carrier_code
        orig_ba = (getattr(e, "booking_agent", "") or "") if e else ba_code
        if not carrier_code:
            print(
                f"[prefilter] skip row {i+1}: carrier={orig_carrier!r} not in booking_agent_master.json",
                flush=True,
            )
            continue
        if not ba_code:
            print(
                f"[prefilter] skip row {i+1}: booking_agent={orig_ba!r} not in booking_agent_master.json",
                flush=True,
            )
            continue
        kept.append(row)
...
def write_xls(rows: List[Dict[str, str]], output_path: str,
               meta_rod: str = "", meta_vat_cost: str = "0%", meta_vat_sell: str = "0%",
               template_path: str = None, entries: Optional[List["NormalizedRateEntry"]] = None):
    ...
    v3.10.6.2: 沃行预过滤 (_PrefilterError 上抛, 不 fallback legacy).
    """
    import os as _os
    tpl = template_path or _os.environ.get("DG_CW_TEMPLATE") or DEFAULT_TEMPLATE_PATH
    if tpl and _os.path.exists(tpl):
        try:
            return _write_xls_with_template(rows, output_path, tpl, meta_rod, meta_vat_cost, meta_vat_sell, entries=entries)
        except _PrefilterError:
            raise
...
def _write_xls_legacy(rows: List[Dict[str, str]], output_path: str,
    ...
    for j, col in enumerate(CW_COLUMNS):
        ...
        # v3.10.6.2: 沃行黄色必填列 7 列; 与 red_cols 大部重叠, legacy 下红字优先级覆盖黄色背景.
        yellow_cols = {"POL", "POD", "P/C", "Carrier", "Booking Agent", "Valid fm", "Valid to"}
        if col in red_cols:
...
def _write_xls_with_template(rows: List[Dict[str, str]], output_path: str, template_path: str,
                              meta_rod: str, meta_vat_cost: str, meta_vat_sell: str,
                              entries: Optional[List["NormalizedRateEntry"]] = None) -> bool:
    ...
    Raises _PrefilterError if沃行预过滤失败 (POL/POD 未匹配).
    """
    import xlwt  # noqa: F401  (may be used by callers; force availability check)
    import xlrd
    from xlutils.copy import copy as _xlu_copy

    # v3.10.6.2: 沃行预过滤 (POL/POD 严格, Carrier/BA 软剔除). 失败抛 _PrefilterError,
    # write_xls() 不 fallback legacy, 由 main() 捕获并写 stderr.
    rows = _prefilter_rows(rows, entries=entries)
    n_data = len(rows)
...
def export_template(output_path: str):
    ...
    v3.10.6.2: 示例 carrier 改为非歧义值 (TS LINES / DXHYSHDS). 旧版本用 SITC,
    但 SITC 在 booking_agent_master 是 ambiguous_codes 之一, 沃行预过滤会跳过,
    模板变成空表 (无示例数据). 用 TS LINES 保证示例行不会被预过滤剔除.
    """
    sample = NormalizedRateEntry()
    sample.pol = "CNSHA"
    sample.pod = "THBKK"
    sample.carrier = "TS LINES"
    sample.of_20 = 350
    sample.of_40 = 650
    sample.of_40hq = 700
...
def main():
    ...
    if args.from_feishu:
        ...
        if args.csv:
            ...
            try:
                write_xls(rows, args.output, entries=entries)  # B3 ROD 留空 (2026-09-19 决策)
            except _PrefilterError as e:
                print(f"[ERROR] 沃行预过滤失败, 不写 .xls: {e}", file=sys.stderr, flush=True)
                return 2
