...
REGION_VALUE_ALIASES = {
    ...
    "红海": "中东",
    "亚洲": "东南亚",
    # D96-fix ROD (2026-09-20): 补齐 12 类线名 → FCL canonical (记录级: 本次 190 回填所需).
    # 这些线名此前未映射 → 派生值落在 FCL 18 选项外, 被下游 select 校验清空 → ROD 仍空.
    # ⚠️ 东北亚线 (1 条) 语义跨 日韩/远东（俄罗斯）/中国 3 选项 → 暂取 日韩, 待业务复核.
    "地中海线": "地中海",
    "中美洲线": "中美洲",
    "欧基线": "欧洲",
    "加拿大线": "北美",
    ...
    "西非线": "非洲",
    "东非线": "非洲",
    # D96-fix ROD (2026-09-20 第二批): 补 7 类港口级线名 alias (老板指示 #2, 防复发).
    # 与第一批 12 类的区别: 这 7 类的港在上一批后仍未被覆盖 (TADES→非洲线 / 红海线 / 加勒比线 等).
    # `其他` (9 港) 语义不明 → 不建 alias, 保持留空.
    "美西线": "北美",
    "加勒比线": "加勒比",
    "非洲线": "非洲",
    "红海线": "中东",
...
def port_to_region(port_code: str) -> str:
    ...
    if not code:
        return ""
    # D96-fix (2026-09-19): 只对"已知港口码"派生区域, 其余 (英文名/中文名/缩写) 一律
    # 返回 "" 不猜. 否则非码值的前 2 字符会被当国家码, 把错误区域写进飞书库
    # (SYDNEY→SY=叙利亚→中东 / LAGOS→LA=老挝→东南亚 / CAIRO→CA=加拿大→北美).
    if len(code) != 5:
        return ""
    try:
        from port_resolver import PortResolver
        resolver = PortResolver()
        info = resolver.code_to_info(code)
    except Exception:
        return ""
    if not info:
        # 仅当 code 确为已知港口码 (by_code ∪ alias_index, 含 alias-only 的
        # IDJKT/JPTYO/INHZA 等) 才回退前 2 字符; 未收录英文名 (LAGOS) 不在集合内 → 不猜.
        known = set(resolver.by_code) | set(resolver.alias_index.values())
